...
describe("FeeController.feeProcessor", () => {
  it("should throw exception when currency not found", async () => {
    ...
    await expect(FeeController.feeProcessor(req)).rejects.toThrow(
      "No fee configuration for NGN transactions."
    );
  });
  ...
  it("should handle flat percentage type", async () => {
    ...
    Fee.findConfigs.mockImplementationOnce(
      ({ feeCurrency, feeLocale, feeEntity, feeProperty }) => {
        ...
        expect(feeProperty).toMatchObject([
          "530191******2903",
          530191,
          "MASTERCARD",
          "GTBANK",
        ...
        return [
          {
            _id: { $oid: "6236ffbe71c295eaa00589fa" },
            feeId: "LNPY1223",
            feeCurrency: "NGN",
            ...
            feeType: "FLAT_PERC",
            feeValue: "50:1.4",
            createdAt: { $date: { $numberLong: "1647771582429" } },
            __v: { $numberInt: "0" },
          },
        ];
      }
