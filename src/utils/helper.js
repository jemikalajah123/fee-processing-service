...
const applyConfiguration = (configuration, amount, bearsFee) => {
	...
	switch (configuration.feeType) {
		case 'FLAT_PERC':
			...
			appliedFee = getAppliedFee(Number(amount),Number(feeValues[1]),Number(feeValues[0]))
			break;
		case 'PERC':
			appliedFee = getAppliedFee(amount=Number(amount),value=Number(configuration.feeValue))
			break;

		case 'FLAT':
			appliedFee = getAppliedFee(amount=Number(amount),flatValue=Number(configuration.feeValue))
			break;
